...
class CounterClockwiseFoodProblem(SearchProblem):
    ...
    def __init__(self, startingGameState, goal=(1, 1)):
        ...
        # in initializing the problem, it's okay if you don't have any additional code
        self.end = False
    ...
    def getNextStates(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if dir == action:
                next_states += [(nextState, action, 1)]
            elif left_hand[dir] == action:
                next_states += [(nextState, action, 1)]



        # next_states.append((nextState, action, cost))
        # next_states = []
        #
        #
        # for action in [Directions.EAST, Directions, Directions.EAST, Directions.WEST]:
...
class ScaryProblem(PositionSearchProblem):
    ...
    def cost_function(self, state):
        ...
        y_state = state[1]
        flag = False
        for x_ghost , y_ghost in self.ghosts:
            if (x_state == x_ghost or x_state == x_ghost + 1 or x_state == x_ghost - 1 ) and (y_state == y_ghost or y_state == y_ghost + 1 or y_state == y_ghost - 1 ):
                return 2147483640
        return 1
